...
class PasswordEditText constructor(context: Context, attrs: AttributeSet? = null) :
     ...
     * 下划线的画笔
     */
    private lateinit var mUnderlinePaint: Paint
     ...
     * 分隔线的画笔
     */
    private lateinit var mDivisionLinePaint: Paint
    ...
    override fun onDraw(canvas: Canvas) {
        // 一个密码的宽度
        mPasswordItemWidth =
            (width - 2 * mBorderStrokeSize - (mPasswordNumber - 1) * mDivisionLineSize) / mPasswordNumber
        ...
        if (mListener != null) {
            ...
            if (password.length >= mPasswordNumber) {
                mListener!!.onPasswordFinish(password)
            } else {
                mListener!!.onPasswordChanged(password)
            }
        }
    }
    ...
    interface OnFinishListener {
        fun onPasswordFinish(password: String?)
